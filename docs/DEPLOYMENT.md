...
1. The three public plans are defined in `src/config/pricing.json`. Create the matching catalog with `npm run billing:setup` (preview only, no API calls), then set `STRIPE_SECRET_KEY` securely in your terminal environment and run `npm run billing:setup -- --apply` in test mode. The script does not read `.env.local` automatically. Live-mode creation additionally requires `--live`. It creates three products and eighteen recurring prices (three tiers × two billing intervals × three credit bundles), reuses matching lookup keys on retry, and stops on a mismatch instead of overwriting any existing allocation. This is a one-time setup per Stripe account/mode, not a build step.
...
The base bundle is unchanged. The 2× bundle receives 10% off its multiplied base payment; the 3× bundle receives 15% off. Both keep the full multiplied credits. Discounts are defined in `src/config/credit-bundles.json`. New bundle prices have separate immutable lookup keys; checkout uses **quantity 1** because the price already contains the complete bundle. Existing prices and subscriptions remain unchanged. Invoice fulfillment still supports historical quantity-based subscriptions. Re-run the catalog setup once after deploying this update to add the twelve bundle prices; do not multiply their Stripe quantities.
...
For the prompt-to-cartoon studio, planning now uses Gemini 3.8 Flash through fal's OpenRouter-backed chat-completions endpoint, and images/video also use fal. Cartoons require only `FAL_KEY` plus the Supabase worker credentials above; they do not read `GEMINI_API_KEY` or require a separate OpenRouter key. Keep the Google key for the existing faceless/legacy script features. See `docs/CARTOON-STUDIO.md` for cartoon-specific rollout and checkpoint recovery.
