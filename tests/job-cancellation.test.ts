...
function database(job: Record<string, unknown> | null = { id:jobId, status:"processing", provider_request_id:"run_owned", operation:"faceless-render", created_at:"2026-09-23T00:00:00Z", settings:{} }) {
  ...
  return { db:{from:()=>chain} as unknown as SupabaseClient, admin:{rpc} as unknown as SupabaseClient, rpc, chain };
}
...
describe("backend job cancellation",()=>{
  ...
  it("never refunds a run with an unknown dispatch outcome without finding it",async()=>{
