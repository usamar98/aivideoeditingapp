...
export async function cancelJob(db: SupabaseClient, admin: SupabaseClient, userId: string, jobId: string) {
  ...
  try {
    ...
    if (!runId) {
      ...
      let inspected = 0;
      for await (const candidate of runs.list({ tag, from: new Date(job.created_at), taskIdentifier: job.operation === "episode-export" ? "episode-pipeline" : job.operation.startsWith("ugc-") ? "ugc-pipeline" : job.operation.startsWith("cartoon-") ? "cartoon-pipeline" : "faceless-pipeline" }, requestOptions)) {
