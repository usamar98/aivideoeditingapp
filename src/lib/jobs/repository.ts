...
export async function getJobs(db: SupabaseClient, userId: string, filter: JobFilter, page = 0): Promise<JobsPageData> {
  ...
  return { page, pageSize, total: count || 0, jobs: (data || []).map((job) => {
    ...
    return {
      ...
      projectHref: info.projectId ? `/studio/${job.operation.startsWith("shorts-") ? "shorts" : job.operation.startsWith("ugc-") ? "ugc" : job.operation.startsWith("cartoon-") ? "cartoons" : "faceless"}/${info.projectId}` : null,
    };
  }) };
}
