...
import { cartoonPlanCredits, cartoonResolution, isDirectCartoonModel, cartoonModels, cartoonRenderCredits, cartoonStyles, type CartoonProjectView, type CartoonScene, type CartoonStory } from "@/lib/cartoons/schema";
...
export function CartoonEditor({ initial, demo }: { initial: CartoonProjectView; demo: boolean }) {
  ...
  const direct = isDirectCartoonModel(project.brief.model);
  const planCredits = cartoonPlanCredits(project.brief);
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow text-primary">Cartoon studio / {project.status}</p><h1 className="editorial mt-3 max-w-3xl text-4xl sm:text-5xl">{project.title}</h1><p className="mt-4 text-sm text-muted-foreground">{cartoonStyles[project.brief.style]} · {project.brief.duration}s · {project.brief.aspectRatio} · {cartoonModels[project.brief.model].name} · {cartoonResolution(project.brief)}{project.brief.audio === false ? " · Silent" : ""}</p></div><div className="flex gap-2"><Button variant="outline" disabled={demo || pending} onClick={() => run("refresh")} aria-label="Refresh cartoon"><RefreshCw /></Button><Button asChild variant="outline"><Link href={demo ? "/studio/cartoons?preview=1" : "/studio/cartoons"}>New cartoon <Plus /></Link></Button></div></div>
    <ol className="my-8 flex flex-wrap gap-5 text-xs text-muted-foreground"><li className="text-primary">01 · Your idea</li><li className={story ? "text-primary" : ""}>02 · {direct ? "Story review" : "Cast & storyboard"}</li><li className={project.outputUrl ? "text-primary" : ""}>03 · Animate & download</li></ol>
    ...
    {!story && <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]"><section className="rounded-3xl border border-border bg-card p-7"><Sparkles className="size-8 text-primary" /><h2 className="editorial mt-5 text-3xl">{direct ? "Let’s shape your story." : "Let’s meet your characters."}</h2><p className="mt-4 whitespace-pre-wrap text-sm leading-7">{project.brief.prompt}</p>{project.brief.references.length > 0 && <p className="mt-4 text-xs text-muted-foreground">Your references: {project.brief.references.map((r) => r.name).join(", ")}</p>}<p className="mt-6 text-sm leading-7 text-muted-foreground">{direct ? "AI will write an editable story with actions and dialogue. This direct-prompt model skips character portraits and scene-image generation. Review the scenes before animation starts." : "AI will write your story, define each character, and create polished reference portraits. You’ll review the cast and edit the scenes before any animation starts."}</p><Button size="lg" className="mt-6" disabled={demo || pending || busy} onClick={() => run("plan")}>{pending ? <LoaderCircle className="animate-spin" /> : <Users />}{direct ? "Create story" : "Create cast & story"} · {planCredits} credits</Button></section><aside className="rounded-3xl border border-primary/20 bg-accent/30 p-7"><p className="eyebrow text-primary">Your production estimate</p><div className="mt-5 space-y-4 text-sm"><p className="flex justify-between"><span>{direct ? "Editable story" : "Cast & editable story"}</span><strong>{planCredits} credits</strong></p><p className="flex justify-between"><span>{project.brief.duration}s animation</span><strong>{cartoonRenderCredits(project.brief)} credits</strong></p><p className="flex justify-between border-t border-border pt-4"><span>Complete film</span><strong>{planCredits + cartoonRenderCredits(project.brief)} credits</strong></p></div><p className="mt-6 text-xs leading-6 text-muted-foreground">Each stage requires your confirmation. Failed or cancelled jobs return that stage’s reserved credits. Successfully completed story or cast design remains charged even if you choose not to animate.</p></aside></div>}
    {story && <>
      <section className="mb-8 rounded-3xl border border-border bg-card p-5 sm:p-7"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow text-primary">Character bible</p><h2 className="mt-2 text-xl font-semibold">Meet your cast</h2></div><span className="rounded-full bg-accent px-3 py-1.5 text-xs text-primary">{direct ? "Text descriptions · continuity may vary" : "Locked references · reused in every scene"}</span></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{story.characters.map((character) => <article key={character.id} className="overflow-hidden rounded-2xl border border-border"><div className="relative aspect-square bg-accent/35">{project.castUrls[character.id] ? <Image src={project.castUrls[character.id]} alt={`Approved reference portrait of ${character.name}`} fill unoptimized sizes="(max-width:640px) 100vw, 30vw" className="object-contain" /> : <div className="flex h-full flex-col items-center justify-center gap-3 text-muted-foreground"><Users className="size-10 text-primary/40" /><p className="text-xs">{direct ? "Text-defined character · no portrait step" : demo ? "Sample character · no generated portrait" : "Portrait unavailable—refresh to retry"}</p></div>}</div><div className="p-4"><h3 className="font-semibold">{character.name}</h3><p className="mt-2 text-xs leading-6 text-muted-foreground">{character.appearance}</p><p className="mt-3 text-xs"><span className="text-primary">Voice direction:</span> {character.voice}</p><p className="mt-2 text-xs text-muted-foreground">{character.referenceSlot ? "Adapted from your uploaded artwork" : "Created from your prompt"}</p></div></article>)}</div></section>
      ...
      <section className="mt-7 rounded-2xl border border-primary/20 bg-accent/35 p-5 sm:p-7"><div className="flex flex-wrap items-center justify-between gap-5"><div><h2 className="text-lg font-semibold">Ready for their first performance?</h2><p className="mt-2 max-w-xl text-xs leading-6 text-muted-foreground">{project.brief.duration}s of {project.brief.audio === false ? "silent animation" : "animation, English dialogue and sound"}. {dirty ? "Your edits will be saved before rendering." : "Review your cast and storyboard before continuing."} Each render costs {cartoonRenderCredits(project.brief)} credits. AI speech and identity consistency can vary.</p></div><Button size="lg" disabled={demo || pending || busy} onClick={() => run("render")}>{pending ? <LoaderCircle className="animate-spin" /> : <Clapperboard />}Animate film · {cartoonRenderCredits(project.brief)} credits</Button></div></section>
    </>}
