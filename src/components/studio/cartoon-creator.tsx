...
import { CARTOON_PLAN_CREDITS, cartoonModels, cartoonRenderCredits, cartoonStyles, type CartoonBrief } from "@/lib/cartoons/schema";
  ...
  function create() {
    ...
    startTransition(async () => {
      try {
        ...
        const result = await createCartoonProject({ prompt, style, duration, aspectRatio, model, references: uploads, rightsConfirmed: rights });
    ...
    <div className="grid items-start gap-8 lg:grid-cols-[1.35fr_1fr]">
      <form className="rounded-3xl border border-border bg-card p-5 sm:p-8" onSubmit={(event) => { event.preventDefault(); create(); }}>
        <fieldset disabled={pending} className="min-w-0 space-y-7">
          ...
          <div className="border-t border-border pt-5"><div className="mb-4 flex flex-wrap justify-between gap-2 text-xs"><span>Cast & story: <strong>{CARTOON_PLAN_CREDITS} credits</strong></span><span>Animation: <strong>{cartoonRenderCredits({ model, duration })} credits</strong></span></div><Button type="submit" size="lg" disabled={demo || pending || !rights || prompt.trim().length < 20} className="w-full rounded-xl">{pending ? <LoaderCircle className="animate-spin" /> : <Sparkles />}{phase || "Build my cartoon"}<ArrowRight /></Button><p className="mt-3 text-center text-xs text-muted-foreground">Saving is free. Confirm credits on the next screen.</p></div>
        </fieldset>
        {error && <p role="alert" className="mt-4 rounded-xl border border-destructive/30 p-3 text-sm">{error}</p>}
      </form>
      <aside className="space-y-5"><div className="overflow-hidden rounded-3xl border border-border bg-card"><div className="relative aspect-[4/3]"><Image src="/demo/weather-workshop.png" alt="Illustrative cartoon scene with two characters in a whimsical workshop" fill sizes="(max-width:1024px) 100vw, 40vw" className="object-cover" /><span className="absolute left-4 top-4 rounded-full bg-card/95 px-3 py-1.5 text-[10px] font-medium uppercase tracking-widest">Illustrative sample · not a live output</span></div><div className="p-6"><p className="eyebrow text-primary">You direct. AI brings it to life.</p><h2 className="editorial mt-3 text-3xl">A little world,<br />all your own.</h2><div className="mt-6 space-y-5">{[["01", "Meet your cast", "AI designs up to three characters, using your uploads when provided."], ["02", "Shape the story", "Review the cast and edit each scene’s action, camera and spoken lines."], ["03", "Let it move", "Animate with native dialogue and sound, then download your private MP4."]].map(([number, title, description]) => <div key={number} className="flex gap-4"><span className="font-mono text-xs text-primary">{number}</span><div><h3 className="text-sm font-semibold">{title}</h3><p className="mt-1 text-xs leading-6 text-muted-foreground">{description}</p></div></div>)}</div></div></div><p className="px-2 text-xs leading-6 text-muted-foreground">AI character consistency, exact dialogue and lip sync can vary. Review the result before publishing. This creates video—not editable 3D rigs. Rendering can take several minutes; you can leave and return.</p></aside>
    </div>
