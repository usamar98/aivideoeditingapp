...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const project = await getUgcProject(db, user.id, id);
    return NextResponse.json(project || { error: "Project not found" }, { status: project ? 200 : 404, headers });
  } catch { return NextResponse.json({ error: "Could not refresh. Your edits are preserved; retry in a moment." }, { status: 503, headers }); }
}
