  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:pt-20">
        <div><Badge variant={feature.status === "published" ? "success" : "warning"}>{feature.status === "published" ? "Available" : "Development preview"}</Badge><h1 className="mt-5 text-4xl font-bold tracking-[-0.045em] sm:text-5xl">{feature.seo.heading}</h1><p className="mt-5 text-lg leading-8 text-muted-foreground">{feature.description}</p><Button asChild size="lg" className="mt-7"><Link href={slug === "faceless-video-generator" ? "/studio/faceless" : "/studio/series/pip-and-moss/episodes/the-cloud-in-a-jar"}>{slug === "faceless-video-generator" ? "Create a faceless video" : "Open the cartoon editor"} <ArrowRight /></Link></Button></div>
